...
class LocationContentTransferSingleSpecialCase(LocationContentTransferCommonCase):
    ...
    * /stock_out_line
    ...
    def test_stock_out_package_split_move(self):
        ...
        stock_issue_inventory = self.env["stock.quant"].search(
            [
                ("location_id", "=", self.content_loc.id),
        ...
        control_inventory = self.env["stock.quant"].search(
            [
                ("location_id", "in", self.content_loc.id),
                ("product_id", "in", self.product_a.id),
                "|",
    ...
    def test_stock_out_line_split_move(self):
        ...
        control_inventory = self.env["stock.quant"].search(
            [
                ("location_id", "in", self.content_loc.id),
                ("product_id", "in", self.product_b.id),
                "|",
                ("inventory_date", ">=", fields.Date.today()),
                ("inventory_quantity_set", "=", True),
            ]
